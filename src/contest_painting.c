...
static void PrintContestPaintingCaption(u8 contestType, bool8 isForArtist)
{
    ...
    if (contestType < MUSEUM_CONTEST_WINNERS_START)
    {
        ...
        StringExpandPlaceholders(gSystemStringVar, gContestHallPaintingCaption);
    }
    else
    {
        // Museum caption
        StringCopy(gStringVar1, gContestPaintingWinner->monName);
        StringExpandPlaceholders(gSystemStringVar, sMuseumCaptions[category]);
    }

    x = GetStringCenterAlignXOffset(FONT_NORMAL, gSystemStringVar, 208);
    AddTextPrinterParameterized(sWindowId, FONT_NORMAL, gSystemStringVar, x, 1, 0, 0);
