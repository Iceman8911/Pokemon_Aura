...
static void GetAwaitingCommunicationText(u8 *dst, u8 activity)
{
    ...
    case ACTIVITY_CONTEST_TOUGH:
        // BUG: argument *dst isn't used, instead it always prints to gSystemStringVar
        // not an issue in practice since Gamefreak never used any other arguments here besides gSystemStringVar
    #ifndef BUGFIX
        StringExpandPlaceholders(gSystemStringVar, sText_AwaitingCommunication);
    #else
...
static void Task_TryBecomeLinkLeader(u8 taskId)
{
    ...
    case LL_STATE_GET_AWAITING_PLAYERS_TEXT:
        ...
        if (GROUP_MIN(sPlayerActivityGroupSize) != 0)
        {
            if (data->playerCount > GROUP_MIN(sPlayerActivityGroupSize) - 1 && GROUP_MAX(sPlayerActivityGroupSize) != 0)
                StringExpandPlaceholders(gSystemStringVar, sText_AwaitingLinkPressStart);
            else
                StringExpandPlaceholders(gSystemStringVar, sText_AwaitingCommunication);
        }
        else
        {
            GetAwaitingCommunicationText(gSystemStringVar, gPlayerCurrActivity);
        }
    ...
    case LL_STATE_PRINT_AWAITING_PLAYERS:
        if (PrintOnTextbox(&data->textState, gSystemStringVar))
            ...
            if (data->joinRequestAnswer == RFU_STATUS_JOIN_GROUP_OK)
            {
                ...
                if (data->playerCount == GROUP_MAX(sPlayerActivityGroupSize))
                {
                    ...
                    else
                    {
                        ...
                        StringExpandPlaceholders(gSystemStringVar, sText_AnOKWasSentToPlayer);
...
static bool8 Leader_SetStateIfMemberListChanged(struct WirelessLink_Leader *data, u32 joinedState, u32 droppedState)
{
    ...
    case UNION_ROOM_SPAWN_IN:
        ...
        Leader_GetAcceptNewMemberPrompt(gSystemStringVar, gPlayerCurrActivity);
...
static void Task_TryJoinLinkGroup(u8 taskId)
{
        ...
        case 0:
            ...
            if (JOY_NEW(A_BUTTON) && id != LIST_NOTHING_CHOSEN)
            {
                ...
                if (data->playerList->players[id].groupScheduledAnim == UNION_ROOM_SPAWN_IN && !data->playerList->players[id].rfu.data.startedActivity)
                {
                    ...
                    {
                        StringCopy(gSystemStringVar, sCantTransmitToTrainerTexts[readyStatus - 1]);
    ...
    case LG_STATE_ASK_JOIN_GROUP:
        GetYouAskedToJoinGroupPleaseWaitMessage(gSystemStringVar, gPlayerCurrActivity);
        if (PrintOnTextbox(&data->textState, gSystemStringVar))
        {
    ...
    case LG_STATE_MAIN:
        ...
        case RFU_STATUS_JOIN_GROUP_OK:
            GetGroupLeaderSentAnOKMessage(gSystemStringVar, gPlayerCurrActivity);
            if (PrintOnTextbox(&data->textState, gSystemStringVar))
            {
                ...
                else
                {
                    ...
                    StringCopy(gStringVar1, sLinkGroupActivityNameTexts[gPlayerCurrActivity]);
                    StringExpandPlaceholders(gSystemStringVar, sText_AwaitingOtherMembers);
                }
            }
            break;
...
static void Task_SendMysteryGift(u8 taskId)
{
    ...
    case 2:
        ...
        break;
    case 3:
        MG_AddMessageTextPrinter(gSystemStringVar);
        data->state = 4;
        break;
    case 4:
    ...
    case 5:
        ...
        break;
    case 7:
        switch (DoMysteryGiftYesNo(&data->textState, &data->yesNoWindowId, FALSE, gSystemStringVar))
        {
        case 0:
            LoadWirelessStatusIndicatorSpriteGfx();
            CreateWirelessStatusIndicatorSprite(0, 0);
...
static void ScheduleFieldMessageWithFollowupState(u32 nextState, const u8 *src)
{
    ...
    if (src != gSystemStringVar)
        StringExpandPlaceholders(gSystemStringVar, src);
}
...
static void Task_RunUnionRoom(u8 taskId)
{
    ...
    case UR_STATE_INTERACT_WITH_PLAYER:
        ...
        case 2: // Ask to join chat
            ScheduleFieldMessageWithFollowupState(UR_STATE_RECV_JOIN_CHAT_REQUEST, gSystemStringVar);
            break;
        }
        break;
        ...
        if (input != LIST_NOTHING_CHOSEN)
        {
            ...
            else
            {
                ...
                if (input == LIST_CANCEL || input == IN_UNION_ROOM)
                {
                    ...
                    StringCopy(gSystemStringVar, sIfYouWantToDoSomethingTexts[gLinkPlayers[0].gender]);
    ...
    case UR_STATE_TRAINER_APPEARS_BUSY:
        StringCopy(gSystemStringVar, sText_TrainerBattleBusy);
    ...
    case UR_STATE_WAIT_FOR_RESPONSE_TO_REQUEST:
        if (!gReceivedRemoteLinkPlayers)
        {
            StringCopy(gSystemStringVar, sText_TrainerBattleBusy); // Redundant, will be copied again in next state
            uroom->state = UR_STATE_TRAINER_APPEARS_BUSY;
        }
        else
        {
            ...
            if (uroom->partnerYesNoResponse == (ACTIVITY_ACCEPT | IN_UNION_ROOM))
            {
                ...
                {
                    ViewURoomPartnerTrainerCard(gSystemStringVar, uroom, FALSE);
            ...
            {
                uroom->state = UR_STATE_REQUEST_DECLINED;
                GetURoomActivityRejectMsg(gSystemStringVar, gPlayerCurrActivity | IN_UNION_ROOM, gLinkPlayers[0].gender);
    ...
    case UR_STATE_PRINT_CARD_INFO:
        if (PrintOnTextbox(&uroom->textState, gSystemStringVar))
        {
    ...
    case UR_STATE_WAIT_FINISH_READING_CARD:
        if (IsLinkTaskFinished())
        {
            if (GetMultiplayerId() == 0)
            {
                ...
                StringExpandPlaceholders(gSystemStringVar, sAwaitingResponseTexts[id]);
    ...
    case UR_STATE_WAIT_FOR_CONTACT_DATA:
        ...
        if (!FuncIsActiveTask(Task_ExchangeCards))
        {
            ...
            id = PlayerHasMetTrainerBefore(gLinkPlayers[1].trainerId, gLinkPlayers[1].name);
            StringExpandPlaceholders(gSystemStringVar, sPlayerContactedYouTexts[id]);
        }
        break;
    ...
    case UR_STATE_HANDLE_CONTACT_DATA:
        ...
        if (HandleContactFromOtherPlayer(uroom) && JOY_NEW(B_BUTTON))
        {
            ...
            StringCopy(gSystemStringVar, sText_ChatEnded);
            uroom->state = UR_STATE_CANCEL_REQUEST_PRINT_MSG;
        }
        break;
    case UR_STATE_RECV_ACTIVITY_REQUEST:
        ScheduleFieldMessageWithFollowupState(UR_STATE_HANDLE_ACTIVITY_REQUEST, gSystemStringVar);
        break;
    case UR_STATE_HANDLE_ACTIVITY_REQUEST:
        ...
        case 0: // ACCEPT
            ...
            if (gPlayerCurrActivity == (ACTIVITY_BATTLE_SINGLE | IN_UNION_ROOM))
            {
                if (!HasAtLeastTwoMonsOfLevel30OrLower())
                {
                    ...
                    StringCopy(gSystemStringVar, sText_NeedTwoMonsOfLevel30OrLower2);
                }
                else
                {
                    Rfu_SendPacket(uroom->playerSendBuffer);
            ...
            {
                Rfu_SendPacket(uroom->playerSendBuffer);
                ViewURoomPartnerTrainerCard(gSystemStringVar, uroom, TRUE);
                uroom->state = UR_STATE_PRINT_CARD_INFO;
            }
            else
            {
        ...
        case MENU_B_PRESSED:
            ...
            Rfu_SendPacket(uroom->playerSendBuffer);
            uroom->state = UR_STATE_DECLINE_ACTIVITY_REQUEST;
            GetYouDeclinedTheOfferMessage(gSystemStringVar, gPlayerCurrActivity);
            break;
        }
        break;
    ...
    case UR_STATE_CANCEL_REQUEST_PRINT_MSG:
        ...
        {
            gPlayerCurrActivity = IN_UNION_ROOM;
            ScheduleFieldMessageWithFollowupState(UR_STATE_CANCEL_REQUEST_RESTART_LINK, gSystemStringVar);
    ...
    case UR_STATE_PRINT_START_ACTIVITY_MSG:
        GetURoomActivityStartMsg(gSystemStringVar, gPlayerCurrActivity | IN_UNION_ROOM);
        ScheduleFieldMessageWithFollowupState(UR_STATE_START_ACTIVITY_LINK, gSystemStringVar);
        break;
    ...
    case UR_STATE_INTERACT_WITH_ATTENDANT:
        ...
        else
        {
            ...
            {
                StringCopy(gSystemStringVar, sText_CancelRegistrationOfEgg);
            }
            else
            {
                ...
                StringExpandPlaceholders(gSystemStringVar, sText_CancelRegistrationOfMon);
            }
            ScheduleFieldMessageWithFollowupState(UR_STATE_CANCEL_REGISTRATION_PROMPT, gSystemStringVar);
        }
        break;
...
static bool32 HandleContactFromOtherPlayer(struct WirelessLink_URoom *uroom)
{
    ...
    {
        s32 id = GetChatLeaderActionRequestMessage(gSystemStringVar, gLinkPlayers[1].gender, &uroom->recvActivityRequest[0], uroom);
        if (id == 0) // Error
        {
            return TRUE;
        }
...
static void PrintGroupMemberOnWindow(u8 windowId, u8 x, u8 y, struct RfuPlayer *player, u8 colorIdx, u8 id)
{
    ...
    ConvertIntToDecimalStringN(gSystemStringVar, id + 1, STR_CONV_MODE_LEADING_ZEROS, 2);
    StringAppend(gSystemStringVar, sText_Colon);
    PrintUnionRoomText(windowId, FONT_NORMAL, gSystemStringVar, x, y, UR_COLOR_DEFAULT);
    x += 18;
    activity = player->rfu.data.activity;
    if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN && !(activity & IN_UNION_ROOM))
    {
        CopyAndTranslatePlayerName(gSystemStringVar, player);
        PrintUnionRoomText(windowId, FONT_NORMAL, gSystemStringVar, x, y, colorIdx);
        ...
        StringCopy(gSystemStringVar, sText_ID);
        StringAppend(gSystemStringVar, trainerId);
        PrintUnionRoomText(windowId, FONT_NORMAL, gSystemStringVar, GetStringRightAlignXOffset(FONT_NORMAL, gSystemStringVar, 0x88), y, colorIdx);
    }
}
...
static void PrintGroupCandidateOnWindow(u8 windowId, u8 x, u8 y, struct RfuPlayer *player, u8 colorIdx, u8 id)
{
    ...
    if (player->groupScheduledAnim == UNION_ROOM_SPAWN_IN)
    {
        ...
        StringCopy(gSystemStringVar, sText_ID);
        StringAppend(gSystemStringVar, trainerId);
        PrintUnionRoomText(windowId, FONT_NORMAL, gSystemStringVar, GetStringRightAlignXOffset(FONT_NORMAL, gSystemStringVar, 0x68), y, colorIdx);
    }
}
...
static s32 UnionRoomGetPlayerInteractionResponse(struct RfuPlayerList *list, bool8 overrideGender, u8 playerIdx, u32 playerGender)
{
    ...
    if (!player->rfu.data.startedActivity && !overrideGender)
    {
        ...
        {
            StringExpandPlaceholders(gSystemStringVar, sJoinChatTexts[metBefore][playerGender]);
            return 2;
        }
        else
        {
    ...
    else
    {
        ...
        {
        case ACTIVITY_BATTLE_SINGLE:
            StringExpandPlaceholders(gSystemStringVar, sBattleReactionTexts[playerGender][Random() % ARRAY_COUNT(sBattleReactionTexts[0])]);
            break;
        case ACTIVITY_TRADE:
            StringExpandPlaceholders(gSystemStringVar, sTradeReactionTexts[playerGender][Random() % 2]);
            break;
        case ACTIVITY_CHAT:
            StringExpandPlaceholders(gSystemStringVar, sChatReactionTexts[playerGender][Random() % ARRAY_COUNT(sChatReactionTexts[0])]);
            break;
        case ACTIVITY_CARD:
            StringExpandPlaceholders(gSystemStringVar, sTrainerCardReactionTexts[playerGender][Random() % ARRAY_COUNT(sTrainerCardReactionTexts[0])]);
            break;
        default:
            StringExpandPlaceholders(gSystemStringVar, sText_TrainerAppearsBusy);
            break;
        }
        return 0;
    }
...
static void ViewURoomPartnerTrainerCard(u8 *unused, struct WirelessLink_URoom *data, bool8 isParent)
{
    ...
    StringCopy(gSystemStringVar, data->trainerCardMsgStrBuffer);
    ...
    StringAppend(gSystemStringVar, data->trainerCardMsgStrBuffer);
