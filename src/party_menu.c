...
static bool8 DisplayCancelChooseMonYesNo(u8 taskId)
{
    ...
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    StringExpandPlaceholders(gSystemStringVar, stringPtr);
    DisplayPartyMenuMessage(gSystemStringVar, TRUE);
...
static void DisplayGaveHeldItemMessage(struct Pokemon *mon, u16 item, bool8 keepOpen, u8 unused)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_PkmnWasGivenItem);
    DisplayPartyMenuMessage(gSystemStringVar, keepOpen);
    ScheduleBgCopyTilemapToVram(2);
}
...
static void DisplayTookHeldItemMessage(struct Pokemon *mon, u16 item, bool8 keepOpen)
{
    GetMonNickname(mon, gStringVar1);
    CopyItemName(item, gStringVar2);
    StringExpandPlaceholders(gSystemStringVar, gText_ReceivedItemFromPkmn);
    DisplayPartyMenuMessage(gSystemStringVar, keepOpen);
    ScheduleBgCopyTilemapToVram(2);
}
...
static void DisplayAlreadyHoldingItemSwitchMessage(struct Pokemon *mon, u16 item, bool8 keepOpen)
{
    GetMonNickname(mon, gStringVar1);
    CopyItemName(item, gStringVar2);
    StringExpandPlaceholders(gSystemStringVar, gText_PkmnAlreadyHoldingItemSwitch);
    DisplayPartyMenuMessage(gSystemStringVar, keepOpen);
    ScheduleBgCopyTilemapToVram(2);
}
...
static void DisplaySwitchedHeldItemMessage(u16 item, u16 item2, bool8 keepOpen)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_SwitchedPkmnItem);
    DisplayPartyMenuMessage(gSystemStringVar, keepOpen);
    ScheduleBgCopyTilemapToVram(2);
}
...
{
    StringExpandPlaceholders(gSystemStringVar, gText_BagFullCouldNotRemoveItem);
}
...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        StringExpandPlaceholders(gSystemStringVar, sActionStringTable[stringId]);
        AddTextPrinterParameterized(*windowPtr, FONT_NORMAL, gSystemStringVar, 0, 1, 0, 0);
        ScheduleBgCopyTilemapToVram(2);
    }
}
...
static bool8 CreateSelectionWindow(u8 taskId)
{
    ...
    else
    {
        ...
        {
            StringExpandPlaceholders(gSystemStringVar, gText_PkmnNotHolding);
            DisplayPartyMenuMessage(gSystemStringVar, TRUE);
            ScheduleBgCopyTilemapToVram(2);
...
static void Task_HandleSwitchItemsYesNoInput(u8 taskId)
{
    ...
    case 0: // Yes, switch items
        ...
        if (AddBagItem(sPartyMenuItemId, 1) == FALSE)
        {
            ...
            DisplayPartyMenuMessage(gSystemStringVar, FALSE);
            gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
        }
        // Giving mail
        else if (ItemIsMail(gSpecialVar_ItemId))
...
static void CursorCb_Toss(u8 taskId)
{
    ...
    {
        CopyItemName(item, gStringVar1);
        StringExpandPlaceholders(gSystemStringVar, gText_ThrowAwayItem);
        DisplayPartyMenuMessage(gSystemStringVar, TRUE);
...
static void Task_HandleTossHeldItemYesNoInput(u8 taskId)
{
    ...
    case 0:
        ...
        StringExpandPlaceholders(gSystemStringVar, gText_ItemThrownAway);
        DisplayPartyMenuMessage(gSystemStringVar, FALSE);
...
static void CursorCb_SendMon(u8 taskId)
{
    ...
    {
        // gSystemStringVar below is the error message buffered by TrySwitchInPokemon
        PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
        DisplayPartyMenuMessage(gSystemStringVar, TRUE);
        gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
    }
...
static void CursorCb_Enter(u8 taskId)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_NoMoreThanVar1Pkmn);
    PlaySE(SE_FAILURE);
    DisplayPartyMenuMessage(gSystemStringVar, TRUE);
    gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
}
...
static void CursorCb_Register(u8 taskId)
{
    ...
    u8 isEventLegal = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_MODERN_FATEFUL_ENCOUNTER);
    ...
    case CANT_REGISTER_MON_NOW:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnCantBeTradedNow);
        break;
    case CANT_REGISTER_MON:
        //StringExpandPlaceholders(gSystemStringVar, gText_PkmnCantBeTraded);
        break;
    case CANT_REGISTER_EGG:
        StringExpandPlaceholders(gSystemStringVar, gText_EggCantBeTradedNow);
        break;
    default:
        PlaySE(SE_SELECT);
        Task_ClosePartyMenu(taskId);
    ...
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
    StringAppend(gSystemStringVar, gText_PauseUntilPress);
    DisplayPartyMenuMessage(gSystemStringVar, TRUE);
    gTasks[taskId].func = Task_ReturnToChooseMonAfterText;
}
...
static void CursorCb_Trade1(u8 taskId)
{
    ...
    {
        StringExpandPlaceholders(gSystemStringVar, sUnionRoomTradeMessages[stringId - 1]);
        PlaySE(SE_FAILURE);
        PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
        PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
        StringAppend(gSystemStringVar, gText_PauseUntilPress);
...
static void CursorCb_Trade2(u8 taskId)
{
    ...
    case CANT_TRADE_LAST_MON:
        StringExpandPlaceholders(gSystemStringVar, gText_OnlyPkmnForBattle);
        break;
    ...
    default: // CAN_TRADE_MON
        ...
        StringExpandPlaceholders(gSystemStringVar, gJPText_AreYouSureYouWantToSpinTradeMon);
        DisplayPartyMenuMessage(gSystemStringVar, TRUE);
    ...
    else
    {
        ...
        else if (sFieldMoveCursorCallbacks[fieldMove].fieldMoveFunc() == TRUE)
        {
            ...
            case FIELD_MOVE_TELEPORT:
                ...
                StringExpandPlaceholders(gSystemStringVar, gText_ReturnToHealingSpot);
            ...
            case FIELD_MOVE_DIG:
                ...
                GetMapNameGeneric(gStringVar1, mapHeader->regionMapSectionId);
                StringExpandPlaceholders(gSystemStringVar, gText_EscapeFromHere);
                DisplayFieldMoveExitAreaMessage(taskId);
                sPartyMenuInternal->data[0] = fieldMove;
                break;
            case FIELD_MOVE_FLY:
...
static void GetMedicineItemEffectMessage(u16 item)
{
    ...
    case ITEM_EFFECT_CURE_POISON:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnCuredOfPoison);
        break;
    case ITEM_EFFECT_CURE_SLEEP:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnWokeUp2);
        break;
    case ITEM_EFFECT_CURE_BURN:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnBurnHealed);
        break;
    case ITEM_EFFECT_CURE_FREEZE:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnThawedOut);
        break;
    case ITEM_EFFECT_CURE_PARALYSIS:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnCuredOfParalysis);
        break;
    case ITEM_EFFECT_CURE_CONFUSION:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnSnappedOutOfConfusion);
        break;
    case ITEM_EFFECT_CURE_INFATUATION:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnGotOverInfatuation);
        break;
    case ITEM_EFFECT_CURE_ALL_STATUS:
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnBecameHealthy);
        break;
    case ITEM_EFFECT_HP_EV:
        StringCopy(gStringVar2, gText_HP3);
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnBaseVar2StatIncreased);
        break;
    ...
    case ITEM_EFFECT_PP_MAX:
        StringExpandPlaceholders(gSystemStringVar, gText_MovesPPIncreased);
        break;
    case ITEM_EFFECT_HEAL_PP:
        StringExpandPlaceholders(gSystemStringVar, gText_PPWasRestored);
        break;
    default:
        StringExpandPlaceholders(gSystemStringVar, gText_WontHaveEffect);
        break;
    }
}
...
void Task_AbilityCapsule(u8 taskId)
{
    ...
    case 0:
        ...
        StringExpandPlaceholders(gSystemStringVar, askText);
        PlaySE(SE_SELECT);
        DisplayPartyMenuMessage(gSystemStringVar, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
        break;
    case 1:
    ...
    case 3:
        PlaySE(SE_USE_ITEM);
        StringExpandPlaceholders(gSystemStringVar, doneText);
        DisplayPartyMenuMessage(gSystemStringVar, 1);
        ScheduleBgCopyTilemapToVram(2);
        tState++;
        break;
...
{
    GetMonNickname(&gPlayerParty[gPartyMenu.slotId], gStringVar1);
    StringExpandPlaceholders(gSystemStringVar, gText_PkmnHPRestoredByVar2);
    DisplayPartyMenuMessage(gSystemStringVar, FALSE);
    ScheduleBgCopyTilemapToVram(2);
...
void ItemUseCB_ReduceEV(u8 taskId, TaskFunc task)
{
    ...
    else
    {
        ...
        {
            if (ev != newEv)
                StringExpandPlaceholders(gSystemStringVar, gText_PkmnFriendlyBaseVar2Fell);
            else
                StringExpandPlaceholders(gSystemStringVar, gText_PkmnFriendlyBaseVar2CantFall);
        }
        else
        {
            StringExpandPlaceholders(gSystemStringVar, gText_PkmnAdoresBaseVar2Fell);
        }
        DisplayPartyMenuMessage(gSystemStringVar, TRUE);
        ScheduleBgCopyTilemapToVram(2);
        gTasks[taskId].func = task;
...
static void Task_LearnedMove(u8 taskId)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_PkmnLearnedMove3);
    DisplayPartyMenuMessage(gSystemStringVar, TRUE);
    ScheduleBgCopyTilemapToVram(2);
...
static void StopLearningMovePrompt(u8 taskId)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_StopLearningMove2);
    DisplayPartyMenuMessage(gSystemStringVar, TRUE);
    ScheduleBgCopyTilemapToVram(2);
...
static void Task_HandleStopLearningMoveYesNoInput(u8 taskId)
{
    ...
    case 0:
        GetMonNickname(mon, gStringVar1);
        StringCopy(gStringVar2, gMoveNames[gPartyMenu.data1]);
        StringExpandPlaceholders(gSystemStringVar, gText_MoveNotLearned);
        DisplayPartyMenuMessage(gSystemStringVar, TRUE);
        if (gPartyMenu.learnMoveState == 1)
        {
...
static void DisplayExpPoints(u8 taskId, TaskFunc task, u8 holdEffectParam)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_PkmnGainedExp);
    DisplayPartyMenuMessage(gSystemStringVar, FALSE);
    ScheduleBgCopyTilemapToVram(2);
    gTasks[taskId].func = task;
}
    ...
    else
    {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            if (holdEffectParam == 0) // Rare Candy
            {
                ...
                StringExpandPlaceholders(gSystemStringVar, gText_PkmnElevatedToLvVar2);
            }
            else // Exp Candies
            {
                ...
                StringExpandPlaceholders(gSystemStringVar, gText_PkmnGainedExpAndElevatedToLvVar3);
            }
...
static void DisplayMonNeedsToReplaceMove(u8 taskId)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_PkmnNeedsToReplaceMove);
    DisplayPartyMenuMessage(gSystemStringVar, TRUE);
    ScheduleBgCopyTilemapToVram(2);
...
static void Task_TryItemUseFormChange(u8 taskId)
{
    ...
    case 5:
        ...
        {
            GetMonNickname(mon, gStringVar1);
            StringExpandPlaceholders(gSystemStringVar, gText_PkmnTransformed);
            DisplayPartyMenuMessage(gSystemStringVar, FALSE);
            ScheduleBgCopyTilemapToVram(2);
            gTasks[taskId].tState++;
        }
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (IsMultiBattle() == TRUE && (slot == 1 || slot == 4 || slot == 5))
    {
        ...
        StringExpandPlaceholders(gSystemStringVar, gText_CantSwitchWithAlly);
        return FALSE;
    }
    if (GetMonData(&gPlayerParty[slot], MON_DATA_HP) == 0)
    {
        GetMonNickname(&gPlayerParty[slot], gStringVar1);
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnHasNoEnergy);
        return FALSE;
    }
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        {
            GetMonNickname(&gPlayerParty[slot], gStringVar1);
            StringExpandPlaceholders(gSystemStringVar, gText_PkmnAlreadyInBattle);
            return FALSE;
        }
    }
    if (GetMonData(&gPlayerParty[slot], MON_DATA_IS_EGG))
    {
        StringExpandPlaceholders(gSystemStringVar, gText_EggCantBattle);
        return FALSE;
    }
    if (GetPartyIdFromBattleSlot(slot) == gBattleStruct->prevSelectedPartySlot)
    {
        GetMonNickname(&gPlayerParty[slot], gStringVar1);
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnAlreadySelected);
        return FALSE;
    }
    ...
    if (gPartyMenu.action == PARTY_ACTION_CANT_SWITCH)
    {
        ...
        StringExpandPlaceholders(gSystemStringVar, gText_PkmnCantSwitchOut);
        return FALSE;
    }
