        ...
        if (UpdateCommunicationCounts(sStatusScreen->groupCounts, sStatusScreen->prevGroupCounts, sStatusScreen->activities, sStatusScreen->rfuTaskId))
        {
            ...
            for (i = 0; i < NUM_GROUPTYPES; i++)
            {
                ...
                if (i != GROUPTYPE_TOTAL)
                    WCSS_AddTextPrinterParameterized(WIN_GROUP_COUNTS, FONT_NORMAL, gSystemStringVar, 12, 30 * i + 8, COLORMODE_WHITE_LGRAY);
                else
                    WCSS_AddTextPrinterParameterized(WIN_GROUP_COUNTS, FONT_NORMAL, gSystemStringVar, 12, 98, COLORMODE_RED);
            }
