...
static void PrintLinkBattleWinsLossesDraws(struct LinkBattleRecord *records)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_TotalRecordWLD);

    x = GetStringCenterAlignXOffset(FONT_NORMAL, gSystemStringVar, 0xD0);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gSystemStringVar, x, 0x11, 0, NULL);
}
...
void ShowLinkBattleRecords(void)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_PlayersBattleResults);

    x = GetStringCenterAlignXOffset(FONT_NORMAL, gSystemStringVar, 208);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gSystemStringVar, x, 1, 0, NULL);
    PrintLinkBattleWinsLossesDraws(gSaveBlock1Ptr->linkBattleRecords.entries);

    StringExpandPlaceholders(gSystemStringVar, gText_WinLoseDraw);
    AddTextPrinterParameterized(gRecordsWindowId, FONT_NORMAL, gSystemStringVar, 0, 41, 0, NULL);
