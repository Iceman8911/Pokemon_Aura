...
void PrintOnTrainerHillRecordsWindow(void)
{
    ...
    for (i = 0; i < NUM_TRAINER_HILL_MODES; i++)
    {
        ...
        secondsFraction = (total * 168) / 100;
        ConvertIntToDecimalStringN(gStringVar3, secondsFraction, STR_CONV_MODE_LEADING_ZEROS, 2);
        StringExpandPlaceholders(StringCopy(gSystemStringVar, gText_TimeCleared), gText_XMinYDotZSec);
        x = GetStringRightAlignXOffset(FONT_NORMAL, gSystemStringVar, 0xD0);
        AddTextPrinterParameterized3(0, FONT_NORMAL, x, y, sRecordWinColors, TEXT_SKIP_DRAW, gSystemStringVar);
        y += 17;
    }
