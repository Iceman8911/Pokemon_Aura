...
static void PrintMessageWithPlaceholders(const u8 *src)
{
    ...
    MoveRelearnerPrintMessage(gSystemStringVar);
}
...
static void HideHeartSpritesAndShowTeachMoveText(bool8 onlyHideSprites)
{
    ...
    if (!onlyHideSprites)
    {
        StringExpandPlaceholders(gSystemStringVar, gText_TeachWhichMoveToPkmn);
        FillWindowPixelBuffer(RELEARNERWIN_MSG, 0x11);
        AddTextPrinterParameterized(RELEARNERWIN_MSG, FONT_NORMAL, gSystemStringVar, 0, 1, 0, NULL);
    }
}
...
static void HandleInput(bool8 showContest)
{
    ...
    case LIST_CANCEL:
        ...
        StringExpandPlaceholders(gSystemStringVar, gText_MoveRelearnerGiveUp);
        MoveRelearnerPrintMessage(gSystemStringVar);
        break;
    default:
        PlaySE(SE_SELECT);
        ...
        StringExpandPlaceholders(gSystemStringVar, gText_MoveRelearnerTeachMoveConfirm);
        MoveRelearnerPrintMessage(gSystemStringVar);
        break;
    }
}
