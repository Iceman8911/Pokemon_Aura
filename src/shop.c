...
static void BuyMenuPrintPriceInList(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != LIST_CANCEL)
    {
        ...
        if (ItemId_GetImportance(itemId) && (CheckBagHasItem(itemId, 1) || CheckPCHasItem(itemId, 1)))
            StringCopy(gSystemStringVar, gText_SoldOut);
        else
            StringExpandPlaceholders(gSystemStringVar, gText_PokedollarVar1);
        x = GetStringRightAlignXOffset(FONT_NARROW, gSystemStringVar, 120);
        AddTextPrinterParameterized4(windowId, FONT_NARROW, x, y, 0, 0, sShopBuyMenuTextColors[COLORID_ITEM_LIST], TEXT_SKIP_DRAW, gSystemStringVar);
    }
}
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (sMartInfo.martType == MART_TYPE_DECOR)
                        StringExpandPlaceholders(gSystemStringVar, gText_Var1IsItThatllBeVar2);
                    else // MART_TYPE_DECOR2
                        StringExpandPlaceholders(gSystemStringVar, gText_YouWantedVar1ThatllBeVar2);

                    BuyMenuDisplayMessage(taskId, gSystemStringVar, BuyMenuConfirmPurchase);
                }
            }
            break;
        }
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_InBagVar1);
    BuyMenuPrint(WIN_QUANTITY_IN_BAG, gSystemStringVar, 0, 1, 0, COLORID_NORMAL);
    tItemCount = 1;
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    BuyMenuPrint(WIN_QUANTITY_PRICE, gSystemStringVar, 0, 1, 0, COLORID_NORMAL);
}
