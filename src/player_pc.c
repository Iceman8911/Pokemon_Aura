...
static void Mailbox_PrintWhatToDoWithPlayerMailText(u8 taskId)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_WhatToDoWithVar1sMail);
    DisplayItemMessageOnField(taskId, gSystemStringVar, Mailbox_PrintMailOptions);
}
...
static void ItemStorage_PrintMenuItem(u8 windowId, u32 id, u8 yOffset)
{
    if (id != LIST_CANCEL)
    {
        ...
        AddTextPrinterParameterized(windowId, FONT_NARROW, gSystemStringVar, GetStringRightAlignXOffset(FONT_NARROW, gSystemStringVar, 104), yOffset, TEXT_SKIP_DRAW, NULL);
    }
}
...
static void ItemStorage_PrintMessage(const u8 *string)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_NORMAL, gSystemStringVar, 0, 1, 0, NULL);
}
...
static void ItemStorage_PrintItemQuantity(u8 windowId, u16 value, u32 mode, u8 x, u8 y, u8 n)
{
    ...
    StringExpandPlaceholders(gSystemStringVar, gText_xVar1);
    AddTextPrinterParameterized(windowId, FONT_NORMAL, gSystemStringVar, GetStringCenterAlignXOffset(FONT_NORMAL, gSystemStringVar, 48), y, 0, NULL);
}
