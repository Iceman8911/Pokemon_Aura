...
static void AskUsePokeblock(void)
{
    ...
    StringCopy(gSystemStringVar, stringBuffer);
    FillWindowPixelBuffer(WIN_TEXT, 17);
    DrawTextBorderOuter(WIN_TEXT, 151, 14);
    AddTextPrinterParameterized(WIN_TEXT, FONT_NORMAL, gSystemStringVar, 0, 1, 0, NULL);
...
static void PrintFirstEnhancement(void)
{
    ...
    if (sInfo->condition < CONDITION_COUNT)
        BufferEnhancedText(gSystemStringVar, sInfo->condition, sInfo->enhancements[sInfo->condition]);
    else
        BufferEnhancedText(gSystemStringVar, sInfo->condition, 0);

    PrintMenuWindowText(gSystemStringVar);
    PutWindowTilemap(WIN_TEXT);
    CopyWindowToVram(WIN_TEXT, COPYWIN_FULL);
}
